...
async def before_questions(message: types.Message,  state: FSMContext):
    ...
    await message.answer("1. Чья ответственность за компрометацию пароля пользователя?", reply_markup=keyboard)
...
async def answer_q1(message: types.Message, state: FSMContext):
    ...
    if answer=="А" or answer=="Б" or answer=="В":
        ...
        await message.answer("2. Может ли злоумышленник начать атаку на офис компании при неправильно опубликованной фотографии офиса, и компания не имеет публичных ресурсов?", reply_markup=keyboard)
...
async def answer_q2(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б":
        ...
        await message.answer("3. Популярный сервис AdBlock можно ли использовать в качестве дополнительного эшелона безопасности?", reply_markup=keyboard)
...
async def answer_q3(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б" or answer == "В":
        ...
        await message.answer("4. Может ли считаться явно юридически значимым документ, к которому нет подписи отдельным файлом?", reply_markup=keyboard)
...
async def answer_q4(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б":
        ...
        await message.answer("5. Будет ли подтверждаться подлинность файла при изменении его названия?", reply_markup=keyboard)
...
async def answer_q5(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б":
        ...
        await message.answer("6. Гарантирует ли наличие соответствия 152-ФЗ у сервиса отсутствия удачных атак на него?", reply_markup=keyboard)
...
async def answer_q6(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б":
        ...
        await message.answer("7. Какому из этих механизмов лучше всего доверять при передачи конфиденциальной информации через мессенджер?", reply_markup=keyboard)
...
async def answer_q7(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б" or answer == "В":
        ...
        await message.answer("8. Внутренние системы шифрования в Excel можно считать надежным?", reply_markup=keyboard)
...
async def answer_q8(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б" or answer == "В":
        ...
        await message.answer("9. Может ли дубликат грифа конфиденциальности быть скрыт при открытии документа?", reply_markup=keyboard)
...
async def answer_q9(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б" or answer == "В":
        ...
        await message.answer("10. Чья задача реагирование на оповещение о заражении антивирусом?", reply_markup=keyboard)
...
async def answer_q10(message: types.Message, state: FSMContext):
    ...
    if answer == "А" or answer == "Б" or answer == "В" or answer == "Г":
        ...
        await message.answer("Поздравляем, ты закончил тест. По его результатам ты получил " + str(db_row[0][4]) + " уровень.")
